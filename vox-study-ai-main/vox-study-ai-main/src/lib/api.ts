...
export const listPages = async (docId: string): Promise<PagesResponse> => {
  ...
  const response = await fetch(`${API_BASE}/docs/${docId}/pages`);
...
export const getExplanation = async (docId: string, pageId: number): Promise<ExplanationResponse> => {
  if (USE_MOCK) {
  ...
  const response = await fetch(`${API_BASE}/docs/${docId}/pages/${pageId}/explain`);
...
export const askQuestion = async (
  ...
  const response = await fetch(`${API_BASE}/docs/${docId}/qa`, {
    method: 'POST',
    headers: {
...
export const getFlashcards = async (docId: string, pageId: number): Promise<FlashcardsResponse> => {
  if (USE_MOCK) {
  ...
  const response = await fetch(`${API_BASE}/docs/${docId}/pages/${pageId}/flashcards`);
...
export const getQuiz = async (docId: string, pageId: number): Promise<QuizResponse> => {
  if (USE_MOCK) {
    await mockDelay(1000);
    return {
  ...
  const response = await fetch(`${API_BASE}/docs/${docId}/pages/${pageId}/quiz`);
...
export const getCheatsheet = async (docId: string, pageId: number): Promise<CheatsheetResponse> => {
  if (USE_MOCK) {
    await mockDelay(800);
    return {
...
3. Example scenario 3`,
  ...
  const response = await fetch(`${API_BASE}/docs/${docId}/pages/${pageId}/cheatsheet`);
